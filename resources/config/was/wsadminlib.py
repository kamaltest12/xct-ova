...
def createGroup(groupName, description) :
    """
    Creates a security group (delete it first if it already exists
    """

    uniqueName = AdminTask.searchGroups(['-cn', groupName])
    if uniqueName:
        AdminTask.deleteGroup(['-uniqueName', uniqueName])

    return AdminTask.createGroup(['-cn', groupName, '-description', description])

#end_def

def addConsoleGroupToRole(groupName, roleName) :
    """
    Adds a security role to a group
    """
    tables = _splitlines(AdminConfig.list('AuthorizationTableExt'))
    for table in tables:
        fileName = AdminConfig.showAttribute(table, 'fileName')
        if fileName == 'admin-authz.xml':
            roleAssignments = _splitlines(AdminConfig.list('RoleAssignmentExt', table))
            for roleAssignment in roleAssignments:
                securityRole = AdminConfig.showAttribute(roleAssignment, 'role')
                securityRoleName = AdminConfig.showAttribute(securityRole, 'roleName')
                if securityRoleName == roleName:
                    # See if the group is already added to this role
                    groupAlreadyAdded = 'false'
                    for groupExt in _splitlines(AdminConfig.list('GroupExt', roleAssignment)) :
                        groupExtName = AdminConfig.showAttribute(groupExt, 'name')
                        if groupExtName == groupName:
                            groupAlreadyAdded = 'true'
                    if groupAlreadyAdded == 'false' :
                        AdminConfig.create('GroupExt', roleAssignment, [['name', groupName]])
...
def removeJAASLoginModules(jaasConfigurationEntry) :

    m = 'removeJAASLoginModules:'

    loginModules = _splitlines(AdminConfig.list('JAASLoginModule', jaasConfigurationEntry))
    alias = AdminConfig.showAttribute(jaasConfigurationEntry, 'alias')

    for loginModule in loginModules:
        loginModuleClassName = AdminConfig.showAttribute(loginModule, 'moduleClassName')
        sop(m, 'removing loginModule %s on alias %s ' % (loginModuleClassName, alias))
        AdminConfig.remove(loginModule)
#end_def

def createJAASLoginModules(jaasConfigurationEntry, jaasLoginModulesList) :
    """
    Removes the current configured login modules and add the new ones
    the jaasLoginModules list should contain the classname and a list of properties list
    """ 

    m = 'createJAASLoginModules:'

    alias = AdminConfig.showAttribute(jaasConfigurationEntry, 'alias')
    removeJAASLoginModules(jaasConfigurationEntry)

    for loginModuleItem in jaasLoginModulesList:
        loginModuleClassName = loginModuleItem[0]
        attrs = [['moduleClassName', loginModuleClassName]]
        attrs.append(['authenticationStrategy', 'REQUIRED'])

        sop(m, 'adding loginModule %s on alias %s ' % (loginModuleClassName, alias))

        module = AdminConfig.create('JAASLoginModule', jaasConfigurationEntry, attrs)
        if loginModuleItem[1]:
            for attr in loginModuleItem[1] :
                AdminConfig.create('Property', module, [['name', attr[0]], ['value', attr[1]]])
